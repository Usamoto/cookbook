//
//  This file holds all the JavaScript that's needed for the 'Read' and 'Share'
//  buttons, as well as the Modal component that shows when you click on the
//  'Share' button.
//

// For the Read-Button, we need a function that redirects the user to the recipe.
function loadRecipe(param) {
  window.location.href = `recipe.html?recipe=${param}`;
}

// For the Share-Button, we need to show the Modal as well as fill it with
// the correct information about the chosen recipe.
function showShareModal(param) {
  ...
  const shareUrl = `${dir}recipe.html?recipe=${param}`;
...
function hideModal() {
  ...
  // We empty the value attribute of the input inside of the Modal, so it doesn't linger in the markup.
  document.getElementById('share-link').setAttribute('value', '');

  // We reset the href attributes on the share buttons.
  ...
  // And we also set the clipboard-message to be hidden (see function below), so it doesn't show
  // when the user clicks on another 'Share' button.
  document.getElementById('clipboard-message').classList.add('hide');
}

// For the 'Copy' button inside the Modal, we want to copy the URL to the user's clipboard,
// which is a little tricky.
function copyToClipboard() {
  // First, we need the value that we want to copy, which is the value of the input element.
  ...
  // We make the textarea non-editable and give it styles that hide it away out of sight.
